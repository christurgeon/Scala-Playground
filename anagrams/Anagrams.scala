...
object Anagrams extends AnagramsInterface:
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match 
    case Nil => List(List()) // empty subset
    case head::tail => 
      val char = head._1
      val frequency = head._2
      val sublists =
        for (
          combos <- combinations(tail); 
          count  <- 1 to frequency
        )
        yield (char, count) :: combos
      sublists ++ combinations(tail)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = y match
    case Nil => x 
    case yhead::ytail =>
      val idx = x.indexWhere(e => e._1 == yhead._1)
      val newx = x.updated(idx, (yhead._1, x(idx)._2 - yhead._2)).filter(e => e._2 > 0)
      subtract(newx, ytail)
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = 
    def helper(occurrences: Occurrences): List[Sentence] = occurrences match
      ...
      case _::_ =>
        for {
          c <- combinations(occurrences) if dictionaryByOccurrences.getOrElse(c, Nil) != Nil
          w <- dictionaryByOccurrences.getOrElse(c, Nil) 
          s <- helper(subtract(occurrences, c))               
         } yield w::s 
